...
- **1.21 数据驱动注册表**：`Enchantment`（附魔）与 `PaintingVariant`（画）不再是普通注册表，加入本包（如 `worldgen/enchantment/`、`worldgen/painting/`）的 `ResourceKey` + bootstrap，由 `ModWorldGenProvider` 的 `RegistrySetBuilder` 一并产出
